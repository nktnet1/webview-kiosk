...
In Lock Task Mode, the only way to unlock {APP_NAME} would be using a methods
...
perform the duties of a DPC. Below are instructions for two common solutions:
...
2. [OwnDroid](https://github.com/BinTianqi/OwnDroid)
   ...
   - Go to `System -> Lock Task Mode -> Applications` and add Webview Kiosk's package
...
prompted to grant permission when the app launches. Please note that Dhizuku
will need to be set as the device owner as per the
[official instructions](https://github.com/iamr0s/Dhizuku/discussions/19).
