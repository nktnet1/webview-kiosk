...
into the setting option (and not the App Pinning toggle/switch) to see
additional screen pinning settings.
...
If your device is fully-managed (company-owned), or you're using the
[Test DPC](https://github.com/googlesamples/android-testdpc) app,
you can make use of [managed configurations](https://developer.android.com/work/managed-configurations)
to restrict the settings of {APP_NAME} or change them remotely from your MDM.

For example, if your device is enrolled with Google Endpoint Management, refer to
[Google's official instructions](https://github.com/h-mdm/hmdm-server/issues/49#issuecomment-115115603A).

Please note that managed configuration APIs can only be used by certified EMM
providers. For example, [Headwind MDM cannot not support this](https://github.com/h-mdm/hmdm-server/issues/49#issuecomment-1151156032).