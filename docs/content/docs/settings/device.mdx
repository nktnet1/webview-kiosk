...
Set to true to allow NFC usage in {APP_NAME} via a Web NFC API compatibility bridge.

This will allow any website to use the devices NFC to read and write tags.

Please note that this is not directly using Web NFC as is is not available in WebView,
rather it is a compatibility layer between the native NFC and JS. This means that it does not have the same 
[security limitations](https://developer.chrome.com/docs/capabilities/nfc#security-and-permissions) 
and therefore does work on insecure websites.

**Default:** false

## 9. Back Button Hold Action
