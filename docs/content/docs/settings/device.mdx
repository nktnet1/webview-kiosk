---
title: Device
description: Rotation, camera, microphone, location, local network, unlock controls
---
...
On Android 17 (API 37) and newer, grant the
[ACCESS_LOCAL_NETWORK](https://developer.android.com/reference/android/Manifest.permission#ACCESS_LOCAL_NETWORK)
permission to let WebView connect to devices and services on the local network.

Use the permission button in this setting to request the permission or open Android App Info to change it later.
This setting is not shown on older Android versions.

**Default:** false

## 8. Allow NFC

Set to true to enable NFC support in {APP_NAME} via a Web NFC compatibility bridge.
...
Set to true to allow {APP_NAME} to send notifications. For example, this
...
## 15. Block Volume Keys

Disable the volume control buttons, specifically:

- [KEYCODE_VOLUME_UP](https://developer.android.com/reference/android/view/KeyEvent#KEYCODE_VOLUME_UP)
- [KEYCODE_VOLUME_DOWN](https://developer.android.com/reference/android/view/KeyEvent#KEYCODE_VOLUME_DOWN)
- [KEYCODE_VOLUME_MUTE](https://developer.android.com/reference/android/view/KeyEvent#KEYCODE_VOLUME_MUTE)
...
### 15.1. Lock Task Features

{APP_NAME} The settings below are extracted from the official documentations:
...
### 15.2. Dhizuku Request Permission on Launch

When enabled, if [Dhizuku](https://github.com/iamr0s/Dhizuku) is installed and
has not granted {APP_NAME} access through the
