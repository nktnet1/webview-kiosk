...
This option injects JavaScript that sets `prefers-color-scheme` according to your selected Webview Kiosk theme.  
...
This option injects JavaScript that sets `document.meta.content` to `width=1024`, simulating a desktop viewport.  
It also listens for JS history state changes (useful for Single Page Applications).

Only enable this if changing the user agent isn't sufficient to force desktop mode, as it may slow down page rendering.
