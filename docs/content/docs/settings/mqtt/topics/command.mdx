...
- `command`: *(string)* the type of command to execute
- `interact`: *(boolean, default true)* whether a user interaction should be triggered
    - this will, for example, restore the brightness if `Web Lifecycle -> Dim Screen on Inactivity (seconds)` is configured
