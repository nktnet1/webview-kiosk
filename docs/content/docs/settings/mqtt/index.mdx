...
When enabled, {APP_NAME} will start a
[foreground service](https://developer.android.com/develop/background-work/services/fgs)
to keep the MQTT connection alive, even when the application goes to the
background or the device screen is turned off.
