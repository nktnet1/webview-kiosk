...
Managed local files use `https://appassets.androidplatform.net` links backed by
`WebViewAssetLoader`. The Manage Local Files screen has a compact `Legacy file:// links`
switch for exposing the older `file://` open/copy actions when required.
